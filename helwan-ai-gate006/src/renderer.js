// renderer.js
// Finalized: Multi-tab enforced + title/url bar support
// NOTE: All in-code comments are in English (per preference).
...
const urlBar = document.getElementById('url-bar'); // may be null if not present in DOM
...
// This runs inside the webview's context via executeJavaScript.
...
// Create a new tab + webview. Each tab uses its own persistent partition for isolation.
function createTab(url, name, displayName = null) {
  ...
  const tabId = name; // unique id passed in
  ...
  // Use unique partition per tab for isolation
  ...
  // Send desktop notification via main process (if available)
...
// Open site helper (kept minimal - creation is done in createTab)
function openSite(url, name) {
  // Deprecated: keeping for backward compatibility if something calls it.
  createTab(url, `${name}-${Date.now()}`, name);
}
